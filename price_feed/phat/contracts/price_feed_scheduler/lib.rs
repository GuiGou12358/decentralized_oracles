...
mod price_feed_scheduler {
    ...
    impl Scheduler {
        ...
        #[ink(message)]
        pub fn poll_1(&self, account_id: AccountId) -> Result<()> {
            pink::info!("Call");
            build_call::<pink::PinkEnvironment>()
                .call(account_id)
                .gas_limit(0)
                .transferred_value(0)
                .exec_input(ExecutionInput::new(Selector::new(ink::selector_bytes!("feed_prices"))))
                //.push_arg(args)
                .returns::<()>()
                .invoke();
            pink::info!("After call");
            Ok(())
        }
        ...
        fn call(&self, job: &JobConfig) -> Result<()> {
            ...
            pink::info!("call contract {:?} - selector {:2x?}", AccountId::from(job.target.clone()), selector.clone());
            build_call::<pink::PinkEnvironment>()
                .call(AccountId::from(job.target.clone()))
                .gas_limit(0)
                .transferred_value(0)
                //.exec_input(ExecutionInput::new(Selector::new(ink::selector_bytes!("feed_prices"))))
    ...
    mod tests {
        ...
        fn it_works() {
            ...
            //ink::env::test::register_contract::<Scheduler>(hash1.as_ref());
            //ink::test::test::register_contract::<price_feed::PriceFeed>(hash2.as_ref());

            // Deploy Scheduler
            let mut scheduler = SchedulerRef::default()
                .code_hash(hash1)
                .endowment(0)
                .salt_bytes([0u8; 0])
                .instantiate();
            ...
            // Deploy Oracle
            let oracle = price_feed::PriceFeedRef::default()
                .code_hash(hash2)
                .endowment(0)
                .salt_bytes([0u8; 0])
                .instantiate();
            ...
            scheduler
                .add_job(
                    ...
                    ink::selector_bytes!("feed_prices").to_vec(),
